...
config = {
	...
	each : function(content, title, messages, page) {
		...
		// 之前就發現於某些條目，會把{{tl|Authority control}}模板加至頁頂。經觀察，大多是因為{{tl|Featured article}}或{{tl|Good article}}在頁頂而出問題。
		// 依[[WP:格式手冊/版面布局#附錄排序]]，{{tl|Featured article}}或{{tl|Good article}}應置放在條目的最底部。之後嘗試將之移至條目的最底部，但在那之前還有些條目尚未改回。由於條目數量過多，現在再跑一次似乎不是好方法。
...
function check_viaf(viafID,callback){
	var viafID=54182;
	CeL.get_URL('https://viaf.org/viaf/'+viafID+'/rdf.xml',function(data){
		data=data.responseText;
		//data is XML
		if(data.includes('<owl:deprecated ')){
			// This VIAF Cluster has been deleted. It is no longer part of VIAF.
			//e.g., 54182 vs. 111507481
...
//https://www.wikidata.org/wiki/Q3907614

//https://www.wikidata.org/wiki/Wikidata:WikiProject_Authority_control/Status
