...
function main_process() {
	...
	board_list.process_index = 0;
	function next_board() {
		if (board_list.process_index === board_list.length) {
			routine_task_done('1d');
			return;
		}

		var board = board_list[board_list.process_index++];
		wiki.page(board, for_board_page.bind({
...
function for_board_page(board_page_data) {
	this.board_page_data = board_page_data;
	var parsed = this.parsed = CeL.wiki.parser(board_page_data).parse();
	if (CeL.wiki.content_of(board_page_data) !== parsed.toString()) {
		// debug 用. check parser, test if parser working properly.
		throw new Error('Parser error: '
				+ CeL.wiki.title_link_of(board_page_data));
	}

	// 按照存檔時的月份建立、歸入存檔頁面。模板參見{{Saved/auto}}
	this.archive_title = board_page_data.title
			+ (new Date).format(archive_page_postfix);

	this.to_exit = parsed.each.exit;

	parsed.each_section(for_each_topic.bind(this));

	wiki.run(remove_topics.bind(this));
}
...
function for_each_topic(topic_token, section_index) {
	...
	// {5,200}: e.g., "討論:標題"
	.test(section_text)) {
		this.archived_topic_list.push(topic_token.range);
		return;
	}
	...
	topic_token.each('template', function(token, index) {
		...
		if (resolved_template_aliases.includes(token.name)) {
			...
			var matched = token.parameters.time
					&& token.parameters.time.match(/^(\d{4})(\d{2})(\d{2})$/);
			if (!matched) {
	...
	if (needless === undefined) {
		...
		if (needless === undefined) {
			...
			CeL.warn('跳過一個日期文字都沒有的討論串 [' + section_title_text + ']，這不是正常的情況。');
			return;
		}
	}
	...
	this.archive_count++;
	this.parsed[topic_token.range[0]] = '\n{{Saved|link=' + archive_title
...
function remove_topics() {
	if (this.error_topics.length > 0) {
		this.error_topics.unshift(CeL.wiki
				.title_link_of(this.board_page_data.title)
				+ ': ' + this.error_topics.length + ' 個議題存檔失敗:');
		CeL.error(this.error_topics.join(']]\n# [['
				+ this.board_page_data.title + '#'));
		this.next_board();
		return;
	}

	var archived_topic_list = this.archived_topic_list, remove_count = 0;
	...
	// 不移除當天存檔者，除非執行第二次。
	// + this.archive_count: 移除當天存檔者
	max_archived_topics)) {
		var topic_slice = archived_topic_list.shift();
		// this.parsed[topic_slice[0] - 1] : section title
	...
	if (this.archive_count === 0 && remove_count === 0) {
		CeL.log(CeL.wiki.title_link_of(this.board_page_data.title)
	...
	var summary_list = [];
	if (remove_count > 0) {
		summary_list.push((monthly_remove_old_notice_section ? '本月首日' : '已存檔'
				+ archived_topic_list.total_count + '個討論串，超過存檔討論串數量上限'
				+ max_archived_topics + '，')
				+ '移除' + remove_count + '個已存檔討論串');
	}
	if (this.archive_count > 0) {
		summary_list.push('存檔' + this.archive_count + '個過期討論串→'
				+ CeL.wiki.title_link_of(this.archive_title));
	}
	...
	CeL.debug('將討論串進行複製、討論內容進行剪切存檔。標記該段落(討論串)為已存檔: '
			+ CeL.wiki.title_link_of(this.board_page_data));
	wiki.page(this.board_page_data).edit(this.parsed.toString(), {
		bot : 1,
		nocreate : 1,
		tags : tags,
		summary : CeL.wiki.title_link_of(
