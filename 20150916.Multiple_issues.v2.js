...
function 處理須合併的條目(page_data, messages) {
	...
	if (多個問題_模板內容.length < 須合併模板數) {
		...
		//
		+ ']，但自條目內容僅抽取出 ' + 多個問題_模板內容.length + '個頁首維護模板，不作合併。'
		//
		+ (章節維護模板_count > 0 ? '另有 '
		//
