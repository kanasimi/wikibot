...
async function get_move_configuration_from_section(meta_configuration, section, options) {
	function get_discussion_link(meta_token) {
		let discussion_link;
		CeL.wiki.parser.parser_prototype.each.call(meta_token, 'link', token => {
			//console.trace(token);
			if (!discussion_link) {
	...
	section.each('<syntaxhighlight>', tag_token => {
		...
		if (task_configuration.replace_tool_page_configuration) {
			// e.g., [[w:zh:Wikipedia:机器人/作业请求#請求台鐵系列內容批量替換]]
			...
			// e.g., [[w:zh:Wikipedia:机器人/作业请求#請求移除WPNL大量信息分發的分類標記]]
			// <syntaxhighlight lang="json">{"replace_tool_configuration":{"get_task_configuration_from":"Category:維基百科社群簡報存檔","namespace":"*"},"replace_tool_page_configuration":{"move_to_link":"DELETE_PAGE","page_list_filter":"!/^Wikipedia:維基百科社群簡報/"}}</syntaxhighlight>
		...
		if (task_configuration.get_task_configuration_from === 'table') {
			if (!task_configuration.list_parser) {
				// CeL.wiki.table_to_array(table)
				task_configuration.list_parser = 'table_to_array';
			}
			if (task_configuration.list_parser !== 'table_to_array') {
				CeL.error(get_move_configuration_from_section.name + ': '
					+ `從表格獲取頁面清單，卻指定 task_configuration.list_parser = ${task_configuration.list_parser}。`);
			}
		}
	...
	if (meta_configuration.get_task_configuration_from === 'table') {
		...
		} else if (meta_configuration.move_configuration_from_page_JSON) {
			for (const category_title of meta_configuration.get_task_configuration_from.split('|')) {
				task_configuration_from_section[category_title] = {
					...meta_configuration.move_configuration_from_page_JSON,
				};
			}
		}
...
function fix_subst_postfix(configuration) {
	...
	configuration.subst_postfix = CeL.preprocessing_filter(configuration.subst_postfix);
	...
	for (let move_configuration_index = 0; move_configuration_index < move_configuration.length; move_configuration_index++) {
		...
		if (task_configuration.replace_text || task_configuration.replace_text_pattern) {
			...
			if (task_configuration.replace_text && !task_configuration.replace_text_pattern) {
				...
				const replace_text_pattern = CeL.preprocessing_filter(task_configuration.replace_text_pattern);
				if (CeL.is_RegExp(replace_text_pattern) && replace_text_pattern.replace)
					task_configuration.replace_text_pattern = replace_text_pattern;
			}
...
function parse_move_link(link, session) {
	...
	if (!session) {
		console.warn(`${parse_move_link.name}: Did not set session!`);
		session = CeL.wiki;
	}
	const link_token = CeL.wiki.parse(`[[${link}]]`, session.append_session_to_options());
	...
	//console.trace([link, link_token]);

	return {
		// page_title: 'ns:page_name'	{{FULLPAGENAME}}
		page_title: normalized_title/* || link_token.page_title */,
		// namespace	{{NAMESPACE}}
		...
		need_check_colon: session.is_namespace(link_token.page_title, ['Category', 'File']),
	};
}
...
async function get_list(task_configuration, list_configuration) {
	...
	if (list_configuration.limit)
		list_options.limit = list_configuration.limit;
	...
	if (Array.isArray(page_list)) {
		...
		if (false) {
			page_list = page_list.filter((page_data) => {
				return !wiki.is_namespace(page_data, ['Wikipedia', 'User'])
	...
	if (list_configuration.page_list_filter) {
		let page_list_filter = list_configuration.page_list_filter;
		if (typeof page_list_filter === 'function') {
			// page_list_filter(page_data, index, page_list)
			page_list = page_list.filter(page_list_filter);
		} else {
			// e.g., [[w:zh:Wikipedia:机器人/作业请求#請求移除WPNL大量信息分發的分類標記]]
			page_list_filter = CeL.preprocessing_filter(page_list_filter);
			page_list = page_list.filter((page_data, index, page_list) => CeL.data.fit_filter(page_list_filter, page_data.title));
		}
	}
	//console.trace(page_list);
	...
	if (/*list_configuration.is_tracking_category && */list_configuration.move_from.ns === wiki.namespace('Category')) {
		page_list.forEach(page_data => {
			if (wiki.is_namespace(page_data, ['Template', 'Module'])) {
...
/**
 * 正規化 title token
 * @param {Array} token title token
 * @returns {String}正規化後的 title token
 */
function normalize_page_title_token(token) {
	return token.toString().trim()
		// [[Wikipedia:Bot作業依頼/過去ログ/2023年6月#鎌ケ谷スタジアムのリンク修正依頼]]
		// 避免 [[ファイターズスタジアム|ファイターズ鎌ヶ谷スタジアム]] → [[ファイターズ鎌ケ谷スタジアム|ファイターズ鎌ヶ谷スタジアム]]
		.replace(/ヶ/g, 'ケ');
}
...
async function main_move_process(task_configuration, meta_configuration) {
	...
	if (task_configuration.also_replace_display_text) {
		...
		task_configuration.also_replace_display_text.forEach(pattern => {
			const replace_to = pattern.replace_to || pattern[1];
			// [[Wikipedia:Bot作業依頼/過去ログ/2023年6月#鎌ケ谷スタジアムのリンク修正依頼]]
			// 避免 [[ファイターズスタジアム|鎌ヶ谷ファイターズスタジアム]]→[[ファイターズ鎌ケ谷スタジアム|鎌ヶ谷ファイターズ鎌ケ谷スタジアム]]
			pattern.add_text_list = CeL.LCS(pattern.source || pattern[0].source, replace_to, {
				diff: true
			}).map(pair => normalize_page_title_token(pair[1]))
				.filter(add_text => add_text.length > 2);
			//console.trace(pattern.source, pattern);
		});
	}
...
function for_each_link(token, index, parent) {
	...
	if (this.keep_display_text) {
		...
		//console.trace(normalize_page_title_token(this.move_to.page_title || token[0]), normalize_page_title_token(token[2]));
		if (/*!this.keep_display_text &&*/ token[2] && !token[1]
			// 假如最後會改變 token[0] → this.move_to.page_title，那麼還是保留 token[2]。 e.g., [[A]] → [[B|A]]
			&& normalize_page_title_token(this.move_to.page_title || token[0]) === normalize_page_title_token(token[2])) {
	...
	if (token[2] && this.also_replace_display_text) {
		...
		this.also_replace_display_text.forEach(pattern => {
			...
			if (token[2].includes(replace_to)) {
				// 文字已包含轉換標的。
				return;
			}
			...
			// [[Wikipedia:Bot作業依頼/過去ログ/2023年6月#鎌ケ谷スタジアムのリンク修正依頼]]
			// 避免 [[ファイターズスタジアム|鎌ヶ谷ファイターズスタジアム]] → [[ファイターズ鎌ケ谷スタジアム|鎌ヶ谷ファイターズ鎌ケ谷スタジアム]]
			if (pattern.add_text_list.some(add_text => normalize_page_title_token(token[2]).includes(add_text))) {
				// e.g., 原先的 display text 即包含了添加的文字 "鎌ヶ谷"。
				return;
			}

			//console.trace(pattern, token);
...
async function subst_template(token, index, parent) {
	...
	//CeL.wiki.evaluate_parser_function_token.call(expanded_code, parse_options);
...
<syntaxhighlight lang="json">{"replace_tool_configuration":{"get_task_configuration_from":"table"}}</syntaxhighlight>
{| class="wikitable"
|-
| [[from]] || [[to]] || <syntaxhighlight lang="json">{"replace_tool_page_configuration":{"namespace":"main"}}</syntaxhighlight>
|}
	...
	// Insert layout template on each page.
	for (const row of rows) {
		let move_from_link, task_configuration_of_row;
		...
		for (const cell of row) {
			...
			CeL.assert([cell.toString(), parsed.toString()],
				// gettext_config:{"id":"wikitext-parser-checking-$1"}
				CeL.gettext('wikitext parser checking: %1', move_from_link || cell.toString()));

			if (!move_from_link || !task_configuration_of_row?.move_to_link) {
				parsed.each('link', token => {
					if (!move_from_link) {
						// row: [page_title, param1, param2, ...]
						const gettext_parameters = [meta_configuration.task_configuration_from_page_JSON.page_title || '%1'].append(row);
						// gettext_parameters: [template, page_title, param1, param2, ...]
						move_from_link = CeL.gettext.apply(null, gettext_parameters);
						const _parsed = CeL.wiki.parser(move_from_link).parse();
						CeL.assert([move_from_link, _parsed.toString()],
							// gettext_config:{"id":"wikitext-parser-checking-$1"}
							CeL.gettext('wikitext parser checking: %1', JSON.stringify(move_from_link)));
						// 有連結就採用連結。
						_parsed.each('link', token => {
							move_from_link = token[0].toString();
							return CeL.wiki.parser.parser_prototype.each.exit;
						});
						return;
					}

					// assert: !!task_configuration_of_row.move_to_link === false
					const move_to_link = token[0].toString();
					if (task_configuration_of_row)
						task_configuration_of_row.move_to_link = move_to_link;
					else
						task_configuration_of_row = { move_to_link };
					return CeL.wiki.parser.parser_prototype.each.exit;
				});
			}
			...
			parsed.each('<syntaxhighlight>', tag_token => {
				...
				try {
					...
					CeL.error(`${parse_task_configuration_from_table.name}: ${e}`);
					return;
				}
				//console.trace(task_configuration);
				...
				if (task_configuration_of_row)
					Object.assign(task_configuration_of_row, task_configuration);
				else
					task_configuration_of_row = task_configuration;
			});

			if (!move_from_link && task_configuration_of_row?.move_from_link)
				move_from_link = task_configuration_of_row.move_from_link;
		}

		if (!move_from_link || !task_configuration_of_row) {
			CeL.warn('Cannot get move_from_link or task_configuration from table row: ' + row.toString());
			continue;
		}
		...
		if (typeof task_configuration_of_row.remove_parameters === 'string') {
			task_configuration_of_row.remove_parameters = task_configuration_of_row.remove_parameters.split('|');
		}
		if (Array.isArray(task_configuration_of_row.remove_parameters)) {
			const remove_parameters__value_filter = Object.create(null);
			task_configuration_of_row.remove_parameters.forEach(parameter_name => {
				if (typeof parameter_name === 'string') {
					...
					if (parameter_name) {
						if (!task_configuration_of_row.replace_parameters) {
							task_configuration_of_row.replace_parameters = Object.create(null);
						}
						task_configuration_of_row.replace_parameters[parameter_name] = CeL.wiki.parse.replace_parameter.KEY_remove_parameter;
					}
				}
			});
			delete task_configuration_of_row.remove_parameters;
			if (!CeL.is_empty_object(remove_parameters__value_filter)) {
				if (!task_configuration_of_row.replace_parameter_options) {
					task_configuration_of_row.replace_parameter_options = Object.create(null);
				}
				if (task_configuration_of_row.replace_parameter_options) {
				...
				task_configuration_of_row.replace_parameter_options = (parameter_name, parameter_value) => {
					if (!(parameter_name in task_configuration_of_row.replace_parameters)) {
		...
		task_configuration_from_section[move_from_link] = task_configuration_of_row;
	}
