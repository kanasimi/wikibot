...
var
/** {Natural}確保不滿足收錄標準的模板已掛上足夠長（至少30日）的時間。 */
limit_days = 30,
/**
...
message_set = {
	notified : '已提醒',
	not_notified : '未提醒用戶',
	do_not_notify : '用戶不想接受不滿足收錄標準的提醒',
...
// e.g., {{Personal announcement|content=請勿在討論頁中加入關注度、動員令、不滿足收錄標準的訊息}}
PATTERN_DO_NOT_NOTIFY = /請?勿(?:在討論頁?中?)?[加放]入[^\n{}]*(?:關注度|收錄標準)/i,
/**
 * 提報不滿足收錄標準的條目中符合此 pattern 者將被 skip。<br />
 * 本bot會以此判斷是否已提報過。若您需要手動提報，可在{{tl|Findsources}}條目後手動加上此註記，即可自動跳過提醒。
 */
PATTERN_SKIP = /於\d{1,2}(?:\.\d+)?天前提報|已經?(?:手動|通知|提醒)/,
/**
 * {String}不滿足收錄標準條目的提報頁面。 [[WP:NP]], [[Wikipedia:關注度/提報]]<br />
 * 2025/1 [[Wikipedia:互助客栈/方针#提議將WP:關注度改名]]: [[Wikipedia:关注度/提报]] →
 * [[Wikipedia:收錄標準/提報]] *
 */
notability_report = 'Wikipedia:收錄標準/提報',
/**
 * 從第一個出現{{Findsources}}的line刪除到第一個未過期的line。<br />
...
function report_notability(page_data) {
	...
	/** {String}不滿足收錄標準的條目內容 關注度不足提報頁面內容 contents */
	不滿足收錄標準的條目內容 = CeL.wiki.content_of(page_data),
	//
	last_title;
	if (!不滿足收錄標準的條目內容)
		throw new Error('No contents get!');
	不滿足收錄標準的條目內容 = 不滿足收錄標準的條目內容.split(/\r?\n/);

	CeL.debug('一行行分析提報不滿足收錄標準的條目內容。');
	不滿足收錄標準的條目內容.forEach(function(line, index) {
		...
		if (last_title in notified_pages) {
			CeL.log('重複提報不滿足收錄標準/已處理過 title: [[' + last_title + ']]。');
			return;
		}
	...
	function each_page(page_data, messages) {
	...
	function after_working(messages, pages) {
		// reset messages.
		messages.length = 0;
		// 對每位用戶一頁頁處理。
		users.forEach(function(user) {
			...
			.forEach(function(title) {
				...
				|| status === message_set.redirected)) {
					// 無須提醒的情況。會把 `notability_report` 中的提報紀錄給刪掉。
					// 執行中不可改變 index，因此僅設為空。
					不滿足收錄標準的條目內容[page_status[title][0]] = '';
			...
			var edit_options = {
				...
				// 若您不想接受不滿足收錄標準提醒，請利用{{bots|optout=afd}}模板。
		...
		// 最終將處理結果寫入 `notability_report` 頁面。
		...
		// || 空白日章節_PATTERN.test(不滿足收錄標準的條目內容.join('\n'))
		) {
			messages.push('移除過期的不滿足收錄標準的條目內容: ' + 移除過期_start_line + '–'
					+ 移除過期_end_line + '/' + 不滿足收錄標準的條目內容.length + '行');
			var date_mark = [];
			不滿足收錄標準的條目內容.splice(移除過期_start_line,
			...
			if (date_mark) {
				...
				不滿足收錄標準的條目內容.splice(移除過期_start_line, 0, date_mark);
			}
			CeL.debug('清理空白無用/重複之日期章節。');
			不滿足收錄標準的條目內容 = 不滿足收錄標準的條目內容.join('\n')
			// 移除無用之日章節。
			...
			CeL.debug('不滿足收錄標準的條目內容:\n' + 不滿足收錄標準的條目內容.slice(0, 600), 2);
			var edit_options = {
				// for .page()
				redirects : 1,
				...
				//
				+ '天前提報不滿足收錄標準的條目，以保持頁面整潔。',
				bot : 1,
				nocreate : 1
			};
			wiki.page(notability_report, edit_options).edit(不滿足收錄標準的條目內容,
					edit_options);
		}

		messages
				.unshift(user_count + ' 用戶 @ ' + (new Date).format('%4Y%2m%2d'));
		var edit_options = {
			// for .page()
			redirects : 1,
