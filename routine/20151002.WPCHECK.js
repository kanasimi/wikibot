// cd /d D:\USB\cgi-bin\program\wiki && node 20151002.WPCHECK.js use_language=zh
...
// [ all, language, text ]
PATTERN_LTR_lang = new RegExp('{{(?:rtl-)?lang\\s*\\|\\s*('
		+ CeL.wiki.LTR_SCRIPTS + ')\\s*\\|\\s*([^{}\\|]+)}}', 'ig');
...
// 必須避免 [[w:zh:Special:Diff/74294709]]
var PATTERN_ISBN = /([^\w\d=])(1[03])?([ ]*\-?)(?:ISBN|\[\[\s*ISBN\s*\]\])[\-\s]*(?:1[03])?[:：\s#.]*([\dx\-\s]{10,})/ig;
